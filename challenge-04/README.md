...
? isTruthy(1);
isTruthy(true);
isTruthy("this is the truth");
isTruthy([]); 
isTruthy({}); 
isTruthy(['alo', 23]);
isTruthy('this is also the truth');
isTruthy(23);
isTruthy("hellow");
isTruthy(2);   
...
?  carro.adicionarPessoa = function (n) {   
    var cota = this.assentos - this.quantidadeDePessoas;
    if (cota > 0) {
        if (cota >= n) {
            this.quantidadeDePessoas += n; 
            return "Já temos " + this.quantidadeDePessoas + " pessoas no carro!";
        } 
        return "Só cabem mais " + cota + " pessoas!"; 
    }
...
? carro.obterMarcaModelo(); //Esse carro é um default default 
